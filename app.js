...
function _initFirebase() {
  ...
  if (typeof firebase === 'undefined') { authResolved = true; window.authUser = null; splashTimerDone = true; evaluateLaunch(); return; }
  ...
  } catch (e) { authResolved = true; window.authUser = null; splashTimerDone = true; evaluateLaunch(); return; }
  authResolveTimer = setTimeout(() => { if (!authResolved) { authResolved = true; window.authUser = null; splashTimerDone = true; evaluateLaunch(); } }, 5000);
  auth.onAuthStateChanged(user => {
    clearTimeout(authResolveTimer);
    window.authUser = user || null; authResolved = true; splashTimerDone = true;
...
function vibrateNow(pattern) {
  ...
  if (window.cordova && navigator.notification && typeof navigator.notification.vibrate === 'function') {
    try { navigator.notification.vibrate(Array.isArray(p) ? p.reduce((a, b) => a + b, 0) : p); return true; } catch (e) { }
  }
...
function showToast(msg) { document.querySelectorAll('.toast').forEach(t => t.remove()); const d = document.createElement('div'); d.className = 'toast'; d.textContent = msg; document.body.appendChild(d); setTimeout(() => d.remove(), 2600); }
...
// ========== УДАЛЕНИЕ РАЗДЕЛА ИСТОРИИ ИЗ ИНТЕРФЕЙСА ==========
function removeHistoryUI() {
  const ho = document.getElementById('historyOverlay'); if (ho) ho.remove();
  document.querySelectorAll('[onclick]').forEach(el => {
    if ((el.getAttribute('onclick') || '').includes('openHistory')) {
      const sec = el.closest('.settings-section') || el.parentElement;
      if (sec) sec.remove();
    }
  });
}
...
async function showAppNotification(title, body, tag) {
  ...
  try {
    if ('serviceWorker' in navigator) {
      const reg = await Promise.race([navigator.serviceWorker.ready, new Promise(res => setTimeout(() => res(null), 1500))]);
      if (reg && reg.showNotification) {
        await reg.showNotification(title, { body: body, icon: './icon.png', badge: './icon.png', tag: tag || 'default' });
        return true;
      }
    }
...
async function testNotification() {
  ...
  const ok = await showAppNotification('🔔 Тест уведомления', 'Уведомления работают!', 'test');
...
async function toggleNotifFromSettings() {
  if (!notifEnabled) {
    ...
    notifEnabled = ok;
    if (ok) {
      const shown = await checkUpcomingNotifications(true);
      const finalShown = shown || (await showAppNotification('🔔 Уведомления включены', 'Напомню о платежах заранее', 'notif-enabled'));
      showToast(finalShown ? 'Уведомления включены' : 'Включено, но показать уведомление не удалось');
...
async function resetPartial() {
  ...
  if (c && !c.paid && !c.completed) { await updateCreditField(c.id, { partial: 0 }); closeAction(); showToast('Частичная оплата сброшена'); haptic(15); }
}
...
async function doDelete() { if (selectedCreditId) { await deleteCredit(selectedCreditId); showToast('Удалено'); haptic(15); } closeDelete(); selectedCreditId = null; }
...
async function handleSave() {
  ...
  try {
    ...
    else { creditData.paid = false; creditData.completed = false; creditData.partial = 0; creditData.lastResetMonth = currentMonthKey; creditData.createdAt = new Date().toISOString(); creditData.userId = authUser.uid; await withTimeout(db.collection('credits').add(creditData), 10000); showToast('Кредит добавлен'); }
...
async function handleCardSave() {
  ...
  try {
    ...
    else { cardData.donePayments = 0; cardData.lastResetMonth = currentMonthKey; cardData.createdAt = new Date().toISOString(); await withTimeout(db.collection('credits').add(cardData), 10000); showToast('Карта добавлена'); }
...
async function doCardDelete() { if (selectedCardId) { await db.collection('credits').doc(selectedCardId).delete().catch(() => { }); showToast('Карта удалена'); haptic(15); } closeCardDelete(); selectedCardId = null; }
...
async function doAuth() {
  ...
  const confirmPass = document.getElementById('authConfirmPassword') ? document.getElementById('authConfirmPassword').value.trim() : '';
  if (isLoginMode) {
  ...
  try {
    ...
    try {
      await withTimeout(db.runTransaction(async (transaction) => { const doc = await transaction.get(usernameRef); if (doc.exists) throw new Error('Логин уже занят'); transaction.set(usernameRef, { uid: createdUser.uid, email: email }); }), 15000);
