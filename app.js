...
if (!authResolved) { authResolved = true; authUser = null; splashTimerDone = true; evaluateLaunch(); }
}, 5000);
...
const COLLAPSE_K='crt_collapse_v1', SOUND_K='crt_sound_v1', HAPTIC_K='crt_haptic_v1', NOTIF_K='crt_notif_v1', BIO_K='crt_biometric_v1';
...
['touchstart','pointerdown','keydown'].forEach(ev => document.addEventListener(ev, () => { _getAudioCtx(); }, { once: true, passive: true }));
function _note(ctx, freq, t0, dur, vol, wave) {
...
if (type === 'pay') { _note(ctx, 880, t, 0.16, 0.22); _note(ctx, 1318.5, t + 0.08, 0.22, 0.2); }
else if (type === 'coins') {
const seq = [1975, 2093, 2349, 2637, 2093, 2794, 3136, 3520];
for (let i = 0; i < seq.length; i++) _note(ctx, seq[i] * (0.99 + Math.random() * 0.02), t + i * 0.05, 0.1, 0.16, 'triangle');
_note(ctx, 1568, t + 0.42, 0.3, 0.14);
_note(ctx, 2093, t + 0.46, 0.25, 0.12, 'triangle');
}
else if (type === 'tick') { _note(ctx, 1400, t, 0.045, 0.06, 'square'); }
else if (type === 'unlock') { _note(ctx, 523.25, t, 0.12, 0.18); _note(ctx, 783.99, t + 0.09, 0.14, 0.18); _note(ctx, 1046.5, t + 0.18, 0.26, 0.2); }
else if (type === 'error') { _note(ctx, 220, t, 0.14, 0.16, 'sawtooth'); _note(ctx, 175, t + 0.12, 0.2, 0.16, 'sawtooth'); }
else if (type === 'swipe') { _whoosh(ctx, t, 0.18); }
else if (type === 'save') { _note(ctx, 659.25, t, 0.1, 0.16); _note(ctx, 880, t + 0.09, 0.12, 0.16); _note(ctx, 1108.7, t + 0.18, 0.2, 0.16); }
else { _note(ctx, 520, t, 0.08, 0.14); }
} catch(e) {}
}
// ========== ВИБРАЦИЯ ==========
...
_netDisabled = true;
try { db.disableNetwork().catch(()=>{}); } catch(e) {}
...
// ========== ВСПОМОГАТЕЛЬНЫЕ ==========
function formatPaymentDate(d){ const months=['января','февраля','марта','апреля','мая','июня','июля','августа','сентября','октября','ноября','декабря']; return `до ${d} ${months[new Date().getMonth()]}`; }
...
// ========== ИСТОРИЯ ПЛАТЕЖЕЙ ==========
async function logPayment(creditId, creditName, type, amount, note) {
if (!db || !authUser) return;
try { await db.collection('paymentHistory').add({ userId: authUser.uid, creditId: creditId || null, creditName: creditName || '', type: type, amount: amount || 0, note: note || '', createdAt: new Date().toISOString() }); } catch(e) {}
}
...
const sortedKeys = Object.keys(groups).sort((a,b) => { const [ay,am]=a.split('-').map(Number); const [by,bm]=b.split('-').map(Number); return (by*12+bm)-(ay*12+am); });
sortedKeys.forEach(key => {
const g = groups[key];
const label = document.createElement('div'); label.className = 'history-group-label';
label.textContent = `${mn[g.date.getMonth()]} ${g.date.getFullYear()}`;
...
switch(item.type) {
case 'paid_full': icon = '✅'; sub = 'Полная оплата'; amountClass = 'minus'; amountText = '-' + fmt(item.amount) + ' ₽'; break;
case 'partial': icon = '💰'; sub = 'Частичный взнос'; amountClass = 'minus'; amountText = '-' + fmt(item.amount) + ' ₽'; break;
case 'unpaid': icon = '↩️'; sub = 'Отменена оплата'; amountClass = 'plus'; amountText = '+' + fmt(item.amount) + ' ₽'; break;
case 'reset_partial': icon = '↺'; sub = 'Сброс частичной оплаты'; break;
case 'card_payment': icon = '💳'; sub = 'Платёж по карте'; amountText = item.amount ? fmt(item.amount) + ' ₽' : ''; break;
case 'card_reset': icon = '↺'; sub = 'Сброс платежей карты'; break;
case 'month_reset': icon = '🔄'; sub = 'Автосброс месяца'; break;
...
<div class="history-main"><div class="history-title">${esc(item.creditName || 'Кредит')}</div><div class="history-sub">${esc(sub)}</div></div>
<div class="history-right">${amountText ? `<div class="history-amount ${amountClass}">${amountText}</div>` : ''}<div class="history-time">${dd}.${String(d.getMonth()+1).padStart(2,'0')} · ${hh}:${mm}</div></div>`;
list.appendChild(el);
});
});
}
...
try {
const snap = await db.collection('paymentHistory').where('userId', '==', authUser.uid).get();
const batch = db.batch(); snap.forEach(doc => batch.delete(doc.ref));
await batch.commit(); showToast('История очищена'); closeHistory();
...
const d = doc.data();
if (d.type === 'creditcard') cards.push({ id: doc.id, ...d });
else credits.push({ id: doc.id, ...d });
});
render(); renderCards();
...
if (c.lastResetMonth) { updates.paid = false; updates.partial = 0; updatedCount++; logPayment(c.id, c.name, 'month_reset', 0, 'Автосброс месяца'); }
...
if (c.lastResetMonth === currentKey) continue;
const updates = { lastResetMonth: currentKey };
if (c.lastResetMonth) { updates.donePayments = 0; updatedCount++; logPayment(c.id, c.name, 'card_reset', 0, 'Автосброс платежей карты'); }
await updateCreditField(c.id, updates);
}
...
if (!('Notification' in window)) { showToast('Уведомления не поддерживаются'); return false; }
if (Notification.permission === 'granted') return true;
if (Notification.permission === 'denied') { showToast('Уведомления заблокированы'); return false; }
...
await reg.showNotification(title, { body, icon: './icon.png', tag: tag || 'default' });
...
if (ls.get('crt_last_notif_date') === todayKey) return;
const today = now.getDate();
const upcoming = [];
credits.forEach(c => { if (c.completed || c.paid) return; const diff = (c.day || 1) - today; if (diff === 0 || diff === 1) upcoming.push({ name: c.name, amount: c.amount, when: diff === 0 ? 'сегодня' : 'завтра' }); });
cards.forEach(c => { const done = c.donePayments || 0; const eff = (c.payments || 0) > 0 ? c.payments : 1; if (done >= eff) return; const diff = (c.day || 1) - today; if (diff === 0 || diff === 1) upcoming.push({ name: c.name, amount: c.amount, when: diff === 0 ? 'сегодня' : 'завтра' }); });
if (upcoming.length > 0) {
const total = upcoming.reduce((s, u) => s + (u.amount || 0), 0);
const names = upcoming.slice(0, 3).map(u => u.name).join(', ');
const more = upcoming.length > 3 ? ` и ещё ${upcoming.length - 3}` : '';
const extra = total > 0 ? `\nВсего: ${fmt(total)} ₽` : '';
showAppNotification('💰 Напоминание о платежах', `${upcoming[0].when}: ${names}${more}${extra}`, 'daily-reminder');
...
if (!active.length) list.innerHTML = `<div class="empty-state"><div class="empty-icon">💳</div><div class="empty-title">Нет активных кредитов</div><div class="empty-sub">Нажмите + чтобы добавить первый</div></div>`;
...
if (compList.classList.contains('open')) { compList.innerHTML = ''; const cfrag = document.createDocumentFragment(); completed.forEach(c => cfrag.appendChild(buildCreditItem(c, cm, cy))); compList.appendChild(cfrag); }
...
let termHtml = '';
if (c.endDate) {
const ed = new Date(c.endDate); const ml = (ed.getFullYear() - cy) * 12 + (ed.getMonth() - cm);
...
if (newPaid) { logPayment(c.id, c.name, 'paid_full', c.amount, ''); playSound('coins'); haptic([30, 50, 30]); }
else { logPayment(c.id, c.name, 'unpaid', c.amount, ''); playSound('swipe'); }
}
});
...
if (everythingPaid && !allPaidShown && !allPaidAlreadyShown) {
allPaidShown = true; sessionStorage.setItem('all_paid_shown', '1');
playSound('coins'); haptic([50, 50, 50]);
...
const hintL = item.querySelector('.hint-left');
const hintR = item.querySelector('.hint-right');
if (!content) return;
let startX = 0, startY = 0, currentX = 0, isSwiping = false, direction = null;
item.addEventListener('touchstart', e => {
const t = e.touches[0]; startX = t.clientX; startY = t.clientY; currentX = 0; isSwiping = false; direction = null;
content.style.transition = 'none';
}, { passive: true });
item.addEventListener('touchmove', e => {
const t = e.touches[0]; const dx = t.clientX - startX; const dy = t.clientY - startY;
if (!isSwiping) {
if (Math.abs(dx) > Math.abs(dy) && Math.abs(dx) > 10) { isSwiping = true; direction = dx > 0 ? 'right' : 'left'; }
...
const prog = Math.min(1, Math.abs(dx) / 90);
if (dx > 0) { hintL.style.opacity = prog; hintR.style.opacity = 0; item.style.background = 'rgba(107,107,107,0.10)'; }
else { hintR.style.opacity = prog; hintL.style.opacity = 0; item.style.background = 'rgba(230,57,70,0.10)'; }
}
}, { passive: true });
...
const creditType = item.dataset.creditType;
let acted = false;
...
if (acted) { playSound('swipe'); haptic(20); }
} else if (direction === 'left' && currentX < -80) {
if (confirm('Удалить этот элемент?')) {
const src = creditType === 'card' ? cards : credits;
const found = src.find(x => x.id === creditId);
await db.collection('credits').doc(creditId).delete().catch(()=>{});
logPayment(creditId, found ? found.name : '', 'deleted', 0, 'Удалено свайпом');
playSound('swipe'); haptic([15, 30, 15]);
showToast('Удалено');
acted = true;
}
}
...
hintL.style.opacity = 0; hintR.style.opacity = 0; item.style.background = '';
...
logPayment(c.id, c.name, 'reset_partial', 0, '');
closeAction(); showToast('Частичная оплата сброшена'); haptic(15);
}
}
...
logPayment(c.id, c.name, 'partial', inp, newPaid ? 'Доплачено до конца' : '');
closeAction();
if (newPaid) { playSound('coins'); haptic([30, 50, 30]); } else playSound('pay');
}
...
async function doDelete(){ if(selectedCreditId){ const c = credits.find(x => x.id === selectedCreditId); await deleteCredit(selectedCreditId); logPayment(selectedCreditId, c ? c.name : 'Кредит', 'deleted', 0, ''); showToast('Удалено'); haptic(15); } closeDelete(); selectedCreditId = null; }
// ========== ФОРМА КРЕДИТА ==========
...
const isEdit = editMode; const saveId = editId; const saveCat = selectedCat;
const endDate = getEndDate(); const resetDay = parseInt(document.getElementById('resetDayInput').value) || null;
_isSaving = true;
const saveBtn = document.getElementById('formSaveBtn');
saveBtn.disabled = true; saveBtn.textContent = 'Сохранение...';
try {
const currentMonthKey = getPeriodKeyForToday(resetDay || 1);
const creditData = { name, amount, day, category: saveCat, endDate, resetDay: resetDay || null };
...
creditData.paid = false; creditData.completed = false; creditData.partial = 0;
creditData.lastResetMonth = currentMonthKey;
creditData.createdAt = new Date().toISOString();
creditData.userId = authUser.uid;
await withTimeout(db.collection('credits').add(creditData), 10000);
logPayment(null, name, 'created', amount, '');
showToast('Кредит добавлен');
}
playSound('save'); haptic(20);
closeForm(); resetForm();
} catch (e) {
console.error(e);
showToast(e.code === 'timeout' ? 'Сервер не отвечает. Попробуйте ещё раз' : 'Ошибка сохранения');
} finally {
_isSaving = false; saveBtn.disabled = false;
...
const preview = document.getElementById('cardDotsPreview');
const wrap = document.getElementById('cardDotsWrap');
...
closeFabMenu();
cardEditMode = false; cardEditId = null;
document.getElementById('cardNameInput').value = '';
document.getElementById('cardAmountInput').value = '';
document.getElementById('cardPaymentsInput').value = '';
document.getElementById('cardDayInput').value = '';
document.getElementById('cardResetDayInput').value = '';
document.getElementById('cardDotsPreview').style.display = 'none';
document.getElementById('cardFormTitle').textContent = 'Кредитная карта';
document.getElementById('cardSaveBtn').textContent = 'Добавить карту';
...
try {
const effectiveResetDay = resetDay || day || 1;
const currentMonthKey = getPeriodKeyForToday(effectiveResetDay);
...
cardData.donePayments = 0; cardData.lastResetMonth = currentMonthKey; cardData.createdAt = new Date().toISOString();
await withTimeout(db.collection('credits').add(cardData), 10000);
logPayment(null, name, 'created', amount, 'Кредитная карта');
...
console.error(e);
showToast(e.code === 'timeout' ? 'Сервер не отвечает' : 'Ошибка: ' + (e.message || 'неизвестная ошибка'));
} finally {
_isCardSaving = false; btn.disabled = false;
btn.textContent = wasEditMode ? 'Сохранить' : 'Добавить карту';
}
}
...
const subs = [];
if (c.amount > 0) subs.push(fmt(c.amount) + ' ₽');
if (c.day > 0) subs.push(getCardPaymentDate(c));
...
const wrap = document.getElementById('cardActionDots');
const progressWrap = document.getElementById('cardActionProgressWrap');
wrap.innerHTML = '';
...
for (let i = 0; i < c.payments; i++) {
const d = document.createElement('div');
d.className = 'payment-dot' + (i < done ? ' done' : '');
d.onclick = () => toggleCardPayment(c.id, i, c.payments, c.donePayments || 0);
wrap.appendChild(d);
}
...
logPayment(cardId, c ? c.name : 'Карта', 'card_payment', c ? c.amount : 0, `Платёж ${newDone} из ${total}`);
if (newDone >= total) { playSound('coins'); haptic([30, 50, 30]); } else playSound('pay');
}
closeCardAction();
}
...
logPayment(selectedCardId, c ? c.name : 'Карта', 'card_reset', 0, 'Ручной сброс');
...
document.getElementById('cardAmountInput').value = c.amount || '';
document.getElementById('cardPaymentsInput').value = c.payments || '';
document.getElementById('cardDayInput').value = c.day || '';
document.getElementById('cardResetDayInput').value = c.resetDay || '';
document.getElementById('cardFormTitle').textContent = 'Редактировать карту';
document.getElementById('cardSaveBtn').textContent = 'Сохранить';
...
async function doCardDelete(){ if(selectedCardId){ const c = cards.find(x => x.id === selectedCardId); await db.collection('credits').doc(selectedCardId).delete().catch(()=>{}); logPayment(selectedCardId, c ? c.name : 'Карта', 'deleted', 0, ''); showToast('Карта удалена'); haptic(15); } closeCardDelete(); selectedCardId = null; }
function renderCards(){
const section = document.getElementById('cardsSection');
const list = document.getElementById('cardList');
document.getElementById('cardsCount').textContent = cards.length ? `${cards.length}` : '';
...
cards.forEach(c => {
const done = c.donePayments || 0;
const total = c.payments || 0;
const effectiveTotal = total > 0 ? total : 1;
const isPaid = done >= effectiveTotal;
const hasMultiPayments = total > 1;
const wrap = document.createElement('div'); wrap.style.background = 'var(--surface)';
const item = document.createElement('div');
item.className = 'credit-item swipeable' + (isPaid ? ' paid' : '');
item.dataset.creditId = c.id; item.dataset.creditType = 'card';
const dueTag = c.day > 0 ? `<span class="credit-date-tag">${getCardPaymentDate(c)}</span>` : '';
...
const controlsHtml = hasMultiPayments
? `<div class="card-pay-row" onclick="event.stopPropagation()">
<button class="card-ctrl-btn minus" style="visibility:${done > 0 ? 'visible' : 'hidden'}" onclick="cardPaymentStep('${c.id}', ${done}, ${total}, -1)">−</button>
<span class="card-ctrl-count">${done}/${total}</span>
<button class="card-ctrl-btn plus" style="visibility:${done < total ? 'visible' : 'hidden'}" onclick="cardPaymentStep('${c.id}', ${done}, ${total}, 1)">+</button>
</div>` : '';
item.innerHTML = `
<div class="swipe-hint hint-left">Редактировать</div>
<div class="swipe-hint hint-right">Удалить</div>
<div class="swipe-content">
<div class="credit-main card-main-row">
<div class="card-name-col">
<div class="credit-name">${esc(c.name)}</div>
<div class="credit-meta">${dueTag}</div>
</div>
${controlsHtml}
</div>
<div class="credit-right">${amtHtml}${badgeHtml}</div>
<div class="check-wrap ${isPaid ? 'checked' : ''}" data-id="${c.id}">
<svg viewBox="0 0 14 14" fill="none" stroke="white" stroke-width="2.5"><polyline points="2,7 6,11 12,3"/></svg>
</div>
</div>`;
...
await db.collection('credits').doc(cardId).update({ donePayments: newDone }).catch(()=>{});
const c = cards.find(x => x.id === cardId);
if (newDone > currentDone) { logPayment(cardId, c ? c.name : 'Карта', 'card_payment', c ? c.amount : 0, 'Отмечено как оплачено'); playSound('coins'); haptic([30, 50, 30]); }
else { logPayment(cardId, c ? c.name : 'Карта', 'unpaid', c ? c.amount : 0, 'Отменена оплата'); playSound('swipe'); }
}
...
const c = cards.find(x => x.id === cardId);
if (delta > 0) { logPayment(cardId, c ? c.name : 'Карта', 'card_payment', c ? c.amount : 0, `Платёж ${newDone} из ${total}`); if (newDone >= total) { playSound('coins'); haptic([30, 50, 30]); } else playSound('pay'); }
else logPayment(cardId, c ? c.name : 'Карта', 'unpaid', c ? c.amount : 0, 'Отменён платёж');
} catch(e) { showToast('Ошибка обновления'); }
}
// ========== НАСТРОЙКИ (ОТДЕЛЬНОЕ ОКНО, СОХРАНЕНИЕ/ОТМЕНА) ==========
function gearClick(e) {
e.stopPropagation();
const btn = document.getElementById('settingsBtn');
const svg = btn.querySelector('svg');
svg.style.animation = 'none'; svg.offsetHeight; svg.style.animation = '';
...
document.getElementById('profileAvatar').textContent = (String(display).charAt(0) || 'М').toUpperCase();
updateEmailStatusUI();
}
...
function takeSettingsSnapshot(){ settingsSnapshot = { theme, amountsVisible, fontSize, collapseEnabled, soundEnabled, hapticEnabled, notifEnabled }; }
function saveSettings(){
ls.set(THEME_K, theme); ls.set(AMT_K, amountsVisible); ls.set(FONT_K, fontSize);
ls.set(COLLAPSE_K, collapseEnabled); ls.set(SOUND_K, soundEnabled);
ls.set(HAPTIC_K, hapticEnabled); ls.set(NOTIF_K, notifEnabled);
showToast('✅ Настройки сохранены');
playSound('save'); haptic(25);
closeSettings();
}
function cancelSettings(){
if (settingsSnapshot) {
theme = settingsSnapshot.theme; amountsVisible = settingsSnapshot.amountsVisible;
fontSize = settingsSnapshot.fontSize; collapseEnabled = settingsSnapshot.collapseEnabled;
soundEnabled = settingsSnapshot.soundEnabled; hapticEnabled = settingsSnapshot.hapticEnabled;
notifEnabled = settingsSnapshot.notifEnabled;
applyTheme(); applyAmounts(); applyFont(); applyCollapseUI();
updateSettingsUI();
}
closeSettings();
}
...
function loadSound(){ const v = ls.get(SOUND_K); if (v !== null) soundEnabled = v === 'true'; }
function toggleSoundFromSettings(){ soundEnabled = !soundEnabled; if (soundEnabled) playSound('save'); updateSettingsUI(); haptic(10); }
function loadHaptic(){ const v = ls.get(HAPTIC_K); if (v !== null) hapticEnabled = v === 'true'; }
function toggleHapticFromSettings(){
hapticEnabled = !hapticEnabled;
_userTapped = true;
if (hapticEnabled) {
if (!('vibrate' in navigator)) showToast('Вибрация не поддерживается в этом браузере');
else { try { navigator.vibrate([80, 40, 80]); } catch(e) {} showToast('Проверьте вибрацию'); }
}
updateSettingsUI();
}
function loadNotif(){ const v = ls.get(NOTIF_K); if (v !== null) notifEnabled = v === 'true'; }
async function toggleNotifFromSettings(){
if (!notifEnabled) {
const ok = await requestNotificationPermission();
notifEnabled = ok;
if (ok) { showToast('Уведомления включены (сохраните настройки)'); checkUpcomingNotifications(); }
...
if (!available) { showToast('Биометрия недоступна на этом устройстве'); return; }
const ok = await registerBiometric();
if (ok) { biometricEnabled = true; ls.set(BIO_K, 'true'); showToast('✅ Биометрия включена'); }
...
const st = document.getElementById('soundSwitchTrack'); if (soundEnabled) st.classList.add('dark-on'); else st.classList.remove('dark-on');
...
else if (notifEnabled) notifStatus.textContent = 'За 1 день до платежа';
...
collapseEnabled = ls.get(COLLAPSE_K) === 'true';
activeCollapsed = ls.get(COLLAPSE_ACTIVE_K) === 'true';
cardsCollapsed = ls.get(COLLAPSE_CARDS_K) === 'true';
applyCollapseUI();
}
function toggleCollapseFromSettings() {
collapseEnabled = !collapseEnabled;
if (!collapseEnabled) { activeCollapsed = false; cardsCollapsed = false; ls.set(COLLAPSE_ACTIVE_K, false); ls.set(COLLAPSE_CARDS_K, false); }
applyCollapseUI(); updateSettingsUI(); haptic(10);
}
function applyCollapseUI() {
const activeHeader = document.getElementById('activeSectionHeader');
const activeArrow = document.getElementById('activeArrow');
const creditList = document.getElementById('creditList');
const activeSection = activeHeader && activeHeader.closest('.section');
if (activeHeader && activeArrow && creditList) {
if (collapseEnabled) {
activeHeader.style.cursor = 'pointer'; activeArrow.style.display = ''; activeHeader.onclick = toggleActiveSection;
creditList.style.display = activeCollapsed ? 'none' : '';
activeArrow.style.transform = activeCollapsed ? 'rotate(-90deg)' : '';
if (activeSection) activeSection.classList.toggle('list-hidden', activeCollapsed);
} else {
activeHeader.style.cursor = 'default'; activeArrow.style.display = 'none'; activeHeader.onclick = null;
creditList.style.display = '';
if (activeSection) activeSection.classList.remove('list-hidden');
}
}
const cardsHeader = document.getElementById('cardsSectionHeader');
const cardsArrow = document.getElementById('cardsArrow');
const cardList = document.getElementById('cardList');
const cardsSection = cardsHeader && cardsHeader.closest('.section');
if (cardsHeader && cardsArrow && cardList) {
if (collapseEnabled) {
cardsHeader.style.cursor = 'pointer'; cardsArrow.style.display = ''; cardsHeader.onclick = toggleCardsSection;
cardList.style.display = cardsCollapsed ? 'none' : '';
cardsArrow.style.transform = cardsCollapsed ? 'rotate(-90deg)' : '';
if (cardsSection) cardsSection.classList.toggle('list-hidden', cardsCollapsed);
} else {
cardsHeader.style.cursor = 'default'; cardsArrow.style.display = 'none'; cardsHeader.onclick = null;
cardList.style.display = '';
if (cardsSection) cardsSection.classList.remove('list-hidden');
}
}
const ct = document.getElementById('collapseSwitchTrack');
if (ct) ct.classList.toggle('dark-on', collapseEnabled);
}
...
// ========== АУТЕНТИФИКАЦИЯ (НЕ ЗАВИСАЕТ) ==========
async function resolveEmailFromNick(nick) {
if (!db) return null;
try { const doc = await withTimeout(db.collection('usernames').doc(nick.toLowerCase()).get(), 8000, 'lookup'); if (doc.exists && doc.data().email) return doc.data().email; return null; }
catch(e) { return null; }
}
...
if (!resolved) { showToast('Не найден'); btn.disabled = false; btn.textContent = 'Войти'; return; }
email = resolved;
}
...
document.getElementById('authBtn').textContent = 'Войти'; document.getElementById('authBtn').disabled = false;
...
monthResetDone = false; credits = []; cards = []; paymentHistory = [];
allPaidShown = false;
render(); renderCards();
...
catch(e) { showToast('Неверный пароль'); return; }
}
try {
...
monthResetDone = false; credits = []; cards = []; window.authUser = null;
allPaidShown = false;
closeDeleteAccount();
document.getElementById('app').classList.remove('visible');
document.getElementById('authScreen').style.display = 'flex';
...
playSound('unlock'); haptic([40, 60, 40]);
loadCredits();
}
...
playSound('error'); haptic([60, 40, 60, 40, 60]);
pinSetBuf = '';
...
const hashed = await hashPin(pinSetBuf);
ls.set(PIN_K, hashed); ls.set(PIN_LEN_K, String(pinSetLen));
document.getElementById('pinSetOverlay').classList.remove('active');
updateSettingsUI(); showToast('ПИН-код установлен');
playSound('unlock'); haptic([40, 60, 40]);
lockPanelOpen = false;
...
function pinSetDel(){ pinSetBuf = pinSetBuf.slice(0, -1); playSound('tick'); fillPinDots('pinSetDots', 'sd', pinSetBuf.length, pinSetLen); }
...
if (!(biometricEnabled && ls.get('crt_bio_cred_id'))) return;
...
function pinLockDel(){ pinLockBuf = pinLockBuf.slice(0, -1); playSound('tick'); fillPinDots('pinLockDots', 'ld', pinLockBuf.length, pinLockLen); }
...
publicKey: {
challenge, rp: { name: 'Мои Кредиты' },
user: { id: userId, name: authUser ? authUser.email : 'user', displayName: authUser ? (authUser.displayName || authUser.email) : 'User' },
...
const challenge = new Uint8Array(32); crypto.getRandomValues(challenge);
await navigator.credentials.get({ publicKey: { challenge, allowCredentials: [{ id: rawId, type: 'public-key' }], userVerification: 'preferred', timeout: 60000 } });
return true;
...
if (ok) { unlockSuccess(); showToast('✅ Вход выполнен'); }
else { showToast('Биометрия не распознана'); playSound('error'); haptic([60, 40, 60, 40, 60]); }
}
async function checkShowBiometricLogin() {
const btn = document.getElementById('biometricLoginBtn'); if (!btn) return;
if (biometricEnabled && ls.get('crt_bio_cred_id') && (await isBiometricAvailable())) btn.style.display = '';
...
loadTheme(); loadFontSize(); loadAmounts(); loadCollapse(); loadSound(); loadHaptic(); loadNotif(); loadBiometric(); setupBackBtn();
selectCat('mortgage'); setDateMode('cal');
...
if (!splashTimerDone) {
splashTimerDone = true;
if (!authResolved) { document.getElementById('splash').classList.add('hidden'); document.getElementById('app').classList.remove('visible'); document.getElementById('authScreen').style.display = 'flex'; }
else evaluateLaunch();
}
}, 1500);
_initFirebase();
...
navigator.serviceWorker.register('./sw.js?v=1.2.04').then(reg => reg.update()).catch(() => {});
});
}
